...
def format_command(name):
    return (
        f"python nf_graphs_batched.py "
        f"--config_name={name}"
    )

# Generate the command for each combination
config_list = [x.replace('config', '').replace('.yaml', '') for x in os.listdir('config/config_DLPFC_FINAL')]
...
for gpu_id, batch in enumerate(gpu_batches):
    script_name = f'scripts/gpu_{gpu_id}.sh'  # Adjust GPU ID to start from 4
    ...
    session_name = f"gpu_{gpu_id + 5}"  # Adjust session name to match GPU ID
    ...
    print(f"Started tmux session {session_name} running {len(batch)} commands sequentially.")
    # Run each .sh 2.5 minutes apart
    time.sleep(150)
