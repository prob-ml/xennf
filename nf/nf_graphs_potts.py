...
if __name__ == "__main__":
    ...
    if config.data.dataset == "DLPFC":
        ...
        edge_index = pyg.nn.radius_graph(positions, r=2.0, loop=True)
    ...
    def model(data):
        ...
        # potts energy is exp(-beta * (z[i] - z[j])^2))
        # factor expects log prob
        penalty = -beta * assignment_diff
        pyro.factor(f"potts", penalty)
